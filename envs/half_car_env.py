...
from utils.normalize_half_car import *
...
class HalfCar(Env):
    ...
    def __init__(self, state_num= 4, steps_per_episode=2000, args=None, disable_matlab=False): 
        ...
        if self.args.state == 'SHADD':
            state_num = 6
        elif self.args.state == 'ADD':
            state_num = 4
        elif self.args.state == 'SHcom':
            state_num = 6
        elif self.args.state == 'control':
            state_num = 8
        elif self.args.state == 'SHaug':
            state_num = 5
        else:
            state_num = 4
        ...
        if disable_matlab == False:
            ...
            self.memory_action = [[0.0]*self.action_size] * 8
            self.memory_buffer = [[0.0]*15] * self.args.window_size
    ...
    def step(self, action):
        ...
        obs_cand = [raw_obs[0], raw_obs[6], raw_obs[1], raw_obs[7], (raw_obs[1] - raw_obs[13]), (raw_obs[7] - raw_obs[19]), raw_obs[24], raw_obs[25], raw_obs[30], raw_obs[31], raw_obs[26], raw_obs[32], raw_obs[13], raw_obs[14], raw_obs[19], raw_obs[20]]

        obs_cand_norm = normalizeObsHalf(self,obs_cand)
        # print(obs_cand_norm)

        if self.args.state == 'SHADD':
            obs = [obs_cand_norm[0], obs_cand_norm[1], obs_cand_norm[2], obs_cand_norm[3], obs_cand_norm[4], obs_cand_norm[5]]
        elif self.args.state == 'ADD':
            obs = [obs_cand_norm[0], obs_cand_norm[1], obs_cand_norm[4], obs_cand_norm[5]]
        elif self.args.state == 'SHcom':
            obs = [obs_cand_norm[2], obs_cand_norm[3], obs_cand_norm[4], obs_cand_norm[5], obs_cand_norm[25], obs_cand_norm[31]]
        elif self.args.state == 'control':
            obs = [obs_cand_norm[7], obs_cand_norm[9], obs_cand_norm[10], obs_cand_norm[11], obs_cand_norm[12], obs_cand_norm[13], obs_cand_norm[14], obs_cand_norm[15]]
        elif self.args.state == 'SHaug':
            obs = [obs_cand_norm[2], obs_cand_norm[3], obs_cand_norm[4], obs_cand_norm[5], (obs_cand_norm[2] - obs_cand_norm[3])]
        else:
            obs = [obs_cand_norm[2], obs_cand_norm[3], obs_cand_norm[4], obs_cand_norm[5]]

        if self.args.state_reconstruction != 'None':
            self.memorizeBuffer(obs_cand_norm)
        else:
            self.memorizeBuffer(obs_cand_norm)
        ...
        reward = 0.
        reward = self.getReward(obs_cand_norm,self.memory_action[0])
        ...
        info['args'] = self.args
        if self.current_road_index == 1.0:
            info['road_type'] = 'rough'
        elif self.current_road_index == 2.0:
            info['road_type'] = 'soft'
        elif self.current_road_index == 3.0:
            info['road_type'] = 'bump'
        else:
            info['road_type'] = 'soft'
    ...
    def _compute(self, action, eng):
        ...
        def convertAction(action):
            ...
            act1 = convertActionInnerFunc((self.state_SH[2] - self.state_SH[3]),action_scale[0])
            act2 = convertActionInnerFunc((self.state_SH[4] - self.state_SH[5]),action_scale[1])
            act3 = convertActionInnerFunc((self.state_SH[6] - self.state_SH[7]),action_scale[1])
        ...
        def convertActionInnerFunc(vel,scale):
            ...
            sat_limit = 1000
            if vel >= 0:
                a = nominal_damping * vel + sat_limit * scale
            else:
                a = nominal_damping * vel - sat_limit * scale
            return a

        if self.args.action_type == "damping_force" and self.args.action_cont == "continuous":
        ...
        while True:
            if i % 5 == 0:
                ...
                self.current_road_index = self.road_index[road_step_cnt][0]
        ...
        if self.args.state_reconstruction != 'None':
            ...
            if self.args.state_reconstruction == 'nn':
                ...
                [[tfl_kk3],[tfl_kk2]] = self.kkf(tfl1,1,'tfl')
                [[trl_kk3],[trl_kk2]] = self.kkf(trl1,1,'trl')
                [[tfr_kk3],[tfr_kk2]] = self.kkf(tfr1,1,'tfr')
                [[trr_kk3],[trr_kk2]] = self.kkf(trr1,1,'trr')
                ...
                state = [
                    ...
                    rl1, rl_kk2, rl_kk3,
                    rr1, rr_kk2, rr_kk3,
                    tfl1, tfl_kk2, tfl_kk3,
                    tfr1, tfr_kk2, tfr_kk3,
                    trl1, trl_kk2, trl_kk3,
                    trr1, trr_kk2, trr_kk3,
                    z1, z_kk2, z_kk3,
                    dphi, phi_kk,
                    dtheta, theta_kk
                ]
    ...
    def getReward(self, obs, action_):
        ...
        ## for checking out
        R = self.args.weight
        if self.args.reward == 'half_acc_vel_ddp_dp':
            return - (obs[6]**2 + obs[7]**2 + R * (obs[8]**2 +obs[9]**2))
        elif self.args.reward == 'test_reward':
            if abs(obs[6]) > 1:
                r1 = -obs[6]**2
            else:
                r1 = 2
            if abs(obs[8]) > 1:
                ...
                r2 = 2
            return (r1 + r2)
        elif self.args.reward == 'half_acc_vel_ddp_pos':
            if abs(obs[6]) < 1:
                r1 = 2
            else:
                r1 = 0
            if abs(obs[8]) < 1:
                ...
                r2 = 0
            return - (obs[6]**2 + obs[7]**2 + obs[8]**2) + (r1 + r2)
        elif self.args.reward == 'half_acc_vel_ddp':
            return - (obs[6]**2 + obs[7]**2 + obs[8]**2)
        elif self.args.reward == 'half_acc_ddp':
            return - (obs[6]**2 + obs[8]**2)
        elif self.args.reward == 'half_acc_vel_dp':
            return - (obs[6]**2 + obs[7]**2 + obs[9]**2)
        elif self.args.reward == 'half_acc_ddp_dp':
            return - (obs[6]**2 + obs[8]**2 + obs[9]**2)
        elif self.args.reward == 'half_body_acc':
            return - (obs[0]**2 + obs[1]**2)
        elif self.args.reward == 'half_body_acc_vel':
            return - (obs[0]**2 + obs[1]**2 + obs[2]**2 + obs[3]**2)
        elif self.args.reward == 'half_body_acc_ddz_ddp':
            return - (obs[0]**2 + obs[1]**2 + obs[6]**2 + obs[8]**2)
        else:
            return -(obs[6]**2 + obs[8]**2)
    ...
    def _reset_matlab(self, eng):
        ...
        if self.args.road_type == "iso8608":
            eng.Road_Generator_Fullcar_symmetric(1.0,0.0)
            ...
            eng.Road_Generator_Fullcar_symmetric(1.0,2.0)
        elif self.args.road_type == "only_rough":
            eng.Road_Generator_Fullcar_symmetric(1.0,1.0)
        elif self.args.road_type == "only_bumper":
            eng.Road_Generator_Fullcar_symmetric(1.0,3.0)
    ...
    def kkf(self,in_,index,position): # kinematic kalman filter
        if index == 0: # body position
            ...
            zk = np.array([[hx[0][0] + 0.5 * acc * T**2],[hx[1][0] + acc * T]])
            xk = hx + K @ (zk - C @ hx)
