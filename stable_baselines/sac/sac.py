    ...
    def learn(self, total_timesteps, callback=None,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                # if info['args'].bump_switching == True: # and info['road_type'] == 'bump'
                #     if self.model_bump == None:
                #         from utils.arg_parser import parseLayersFromArgs

                #         layers = parseLayersFromArgs(args=info['args'])
                ...
                #         _bump_args = vars(info['args'])
                #         _bump_args['prefix'] = info['args'].prefix + "-bump"
                #         _bump_args = Namespace(**_bump_args)

                #         bestRewardCallback = getBestRewardCallback(_bump_args)

                #         self.model_bump = SAC(MlpPolicy, self.env, verbose=1, tensorboard_log=os.path.join("tensorboard_"+_bump_args.prefix), policy_kwargs=policy_kwargs)
                #         self.model_bump_learn_gen = self.model_bump.learnPerStep(total_timesteps=20*1000000, log_interval=100, callback=bestRewardCallback)

                #     next(self.model_bump_learn_gen)
                #     r = self.model_bump_learn_gen.send([new_obs, reward, done, info])
