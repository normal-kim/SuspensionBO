...
from envs.fullcar_like_env import HalfCar
...
def main(args):
    ...
    for i in range(test_runs):
        print(i+1,"/",test_runs)
        while True:
            action_l, _states_l = model.predict(obs_l)
            action_r, _states_r = model.predict(obs_r)
            obs_l, obs_r, dones, info = env.step(action_l, action_r)
